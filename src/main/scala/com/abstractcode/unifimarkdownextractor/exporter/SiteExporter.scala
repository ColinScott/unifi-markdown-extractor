...
class FileSiteExporter[F[_] : Monad : Sync](basePath: Path, uniFiApi: UniFiApi[F], authCookies: AuthCookies) extends SiteExporter[F] {
  ...
  def createBaseDirectory: F[Unit] = for {
    exists <- Sync[F].delay(Files.exists(basePath))
    isDirectory <- Sync[F].delay(Files.isDirectory(basePath))
    _ <- Applicative[F].pure(()).ensure(DirectoryCreationFailure(basePath, exists, isDirectory))(_ => !exists || isDirectory)
    _ <- if (exists) Applicative[F].pure(()) else Sync[F].delay(Files.createDirectory(basePath))
  } yield ()
